...
- **对外公开页**：`/u/<地址>`、`/t/<地址>`（地址可自定义，默认是用户名 / `team-<编号>`），不需要登录、按分组归类、带搜索，默认 `noindex`。
...
要换端口或监听地址，看下面的 **[启动](#启动)** 一节（`./run.sh 8080`）。
...
它会造出两个团队、两个个人空间、7 条链接（6 条公开 1 条不公开），覆盖分组、共享、复制、两级权限。**README 下面那些截图就是这套数据**。
...
./run.sh --port 8080            # 写全一点
./run.sh --port=8080            # 也行
./run.sh 8080 --host 0.0.0.0    # 换监听地址（默认 127.0.0.1）
PORT=8080 ./run.sh              # 环境变量也认
./run.sh --help                 # 用法
```

取值的优先级：**命令行 > 环境变量 > 默认**（`PORT=3199 ./run.sh 3161` 走的是 3161）。

脚本在几个地方会拦住你，别嫌它啰嗦 —— 这几条都是真踩过的：

| 情况 | 表现 |
| --- | --- |
| 参数不认识（`--bogus`、`abc`、`3153 3154`） | 非零退出 + 打印用法。**不会静默忽略** —— `./run.sh 8080` 早先就是被无声无视的，然后照旧在 3030 上起；3030 若被占，你拿到的是一个跟端口毫不相干的报错 |
| 端口不合法（`0`、`99999`、`8080x`） | 非零退出，说明得是 1–65535 的数字 |
| 端口被占 | 报出占它的 PID，并给出 `./run.sh <端口+1>` 和 `kill <PID>` 两条下一步 |
| 缺 `.venv` | 提示先建虚拟环境，不再往下跑 |
| 想给同网段的手机看 | `./run.sh --host 0.0.0.0`（⚠️ 别在公网机器上这么开） |
| 想绑 IPv6 | `./run.sh --host ::1` —— 回显的地址会自动带方括号（`http://[::1]:3030`） |

`--reload` 是开着的：改 Python 代码会自动重启。**但它只盯代码文件** ——
改完 `.env` 必须自己重启服务，配置只在进程启动时读一次。

`PORT` / `HOST` 写进 `.env` **不生效**（`.env` 只有应用自己读，`run.sh` 读的是它的参数和 shell 环境），
所以它们没有出现在 `.env.example` 里。
...
不在 `.env` 里、而是启动参数的（`PORT` / `HOST`）：写法见上面的 **[启动](#启动)** 一节。
...
| `/u/<地址>` | 个人对外公开页（默认用户名，可自定义） | 否 |
| `/t/<团队地址>` | 团队对外公开页 | 否 |
| `/help` | **使用指南**：站点功能的完整说明（账号、个人/团队空间、可见性、公开页、权限、FAQ） | 否 |

首页刻意不是登录页：访客先看到有哪些公开空间可以逛。右上角会读当前会话——已登录显示**头像 + 昵称**（点了进 `/app`），未登录才显示「登录」按钮。

**右上角账号区右边有个小问号，进 `/help` 使用指南。** 它做在 `ui.js::accountArea` 里，所以 `/app`、首页、个人公开页、团队公开页**四个顶栏都有**；登录页没有顶栏，入口放在卡片底部那行。指南正文是 `help.html` 里的静态 HTML（脚本挂了也能读），`help.js` 只负责填顶栏。
...
**空间页副标题后面会缀一个「公开页」胶囊**（`公开页 http://…/u/xxx ↗`），点了新标签页打开。只在**该空间开了公开页总开关时**才出现——接口没开时返回 `public_url: null`，`h()` 会把 null 过滤掉，所以直接把它放进 `sub` 数组即可（`sub` 既收字符串也收数组）。这个胶囊**跟副标题同一行**，不另起一行：`layout()` 的 `.page-head` 是 `flex + align-items: flex-end`，在它内部多塞一行会把右上角的按钮一起拽到底部。
...
| 概念 | 含义 |
| --- | --- |
| **共享 = 引用同一份** | 个人链接放进团队列表后，团队看到的就是**同一个链接**。本人在个人空间改了标题，团队侧同步变化。入口是编辑弹窗里的团队勾选（由内向外）。 |
| **复制 = 独立副本** | 反方向（由外向内）：团队里的链接——包括别人共享进来的——用卡片上的「复制到我的空间」按钮拿一份进自己的空间，之后互不影响（会标记「来自团队复制」）。 |
...
- **标签功能已下线**（界面层面）。建/改链接都不提交这个字段，界面上也不再显示标签——卡片、筛选栏、首页空间卡、搜索提示里都没有了。

  数据库里的 `links.tags` 字段和接口返回**保持不动**：已有的标签数据还在（编辑保存也不会被抹掉），只是不再渲染。想哪天恢复，把渲染加回来即可；想彻底退役，那是后端一行 SQL 的事。
...
| --- | --- |
| 首页（空间目录） | `/` |
| 个人公开页 | `/u/<地址>`（默认用户名，可自定义） |
| 团队公开页 | `/t/<团队地址>`（拥有者自定义，如 `doc-team`；留空则用 `team-<编号>`） |

**首页 = 公开空间目录**：访客进来先看到「团队空间 / 个人空间」两个分区，每个空间一张卡片（名称、维护者、链接数），点进去就是那个空间的公开页。

目录里**只列出至少有 1 条公开链接的空间**——开启了总开关但一条都没勾的空间不出现，免得点进去是空页。

**页面形态**：按分组归类，带搜索框（前端筛选，几百条无压力），只读、卡片式、响应式。
...
链接条数（`共 N 条`）放在**搜索框右边**——它和搜索是一件事，筛选时会变成「筛选出 X / Y 条」，跟着搜索框走才看得出因果关系。

**卡片右下角有个「⧉ 备注」按钮**：一键把这条链接的备注复制走，点完图标变对勾一秒半再还原（能看出复制的是哪一张卡）。没有备注的卡片不显示这个按钮。

按钮上直接写出「备注」两个字，是因为光一个复制图标容易被当成「复制这个链接」；两个字把职责说清楚了。反馈时只换图标和颜色、文字不变，所以按钮宽度不会跳。

为什么需要它：整张卡是一个 `<a>`，浏览器默认给它 `draggable=true`，**在卡片上拖选文字会变成「拖链接」**——所以以前想取走卡片里的信息很别扭。

**卡片上不显示任何形式的网址**——常显的域名行（`hicar.cool`）去掉了，悬停浮出的完整地址也**不做**。

域名行是重复信息（卡片自己就是超链接）。而悬停浮出的地址更糟：它**紧挨着右下角的「复制备注」按钮，会让人以为复制的是那个链接**——这个歧义比"省一次右键"代价大得多。

想确认一条卡片指向哪，用浏览器自己的机制：**悬停链接时左下角状态栏会显示完整地址**，右键也有「复制链接地址」。它们离卡片很远，不会跟复制按钮混起来。按钮自己的悬停提示是「复制备注」。

实现上两个坎（都在 `public.js` 里写了注释）：按钮在 `<a>` 内部，必须自己 `preventDefault + stopPropagation`，否则点复制会顺带打开链接；用 `<span role="button">` 而不是 `<button>`，因为 `<a>` 的内容模型不允许出现交互内容。反馈不用 `toast()`——公开页没有 `#toast-root`（那是主应用才有的），会直接报错。

**个人公开页的地址可以改**：在「设置 → 对外公开页」里填一个地址（小写字母/数字/连字符，2-40 位），留空就用用户名。团队公开页同样可改（在团队设置里）。
...
- 筛选条 / 分页器这类「该不该渲染」的三元表达式返回 `null` 时 → 页面上多一个 null
